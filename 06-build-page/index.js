...
async function replaceTemplates(pathToTemplate, pathToComponents, newPath) {
  ...
  for (const component of components) {
    if (component.isFile() && path.extname(component.name) === '.html') {
      const componentContent = await readFile(path.join(pathToComponents, component.name), {encoding: 'utf-8'});
  ...
  for (const templatesVariable of templatesVariables) {
    template = template.replace(templatesVariable, templates.get(`${templatesVariable.slice(2, -2)}.html`));
  }
...
async function mergeStyles(pathToDir, newPathToDir) {
  ...
  const writableStream = fs.createWriteStream(path.join(newPathToDir, 'style.css'));
...
async function build() {
  ...
  replaceTemplates(path.join(__dirname, 'template.html'), path.join(__dirname, 'components'), path.join(__dirname, 'project-dist', 'index.html'));
